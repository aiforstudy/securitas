...
import { Chevron as ChevronDayPicker, DayPicker, DayPickerProps, Dropdown as DropDownDayPicker } from "react-day-picker"
...
export type CalendarProps = DayPickerProps &
	PropsBase & {
		selected?: Pick<CalendarModeProps, "selected">
...
const Calendar: React.FC<CalendarProps> = ({
	...
	const _dayButtonClassName = cn(
		...
		"size-9 rounded-md p-0 text-sm font-normal transition-none aria-selected:opacity-100",
	...
	const Chevron = React.useCallback(({ orientation, ...props }: React.ComponentProps<typeof ChevronDayPicker>) => {
		...
		({ value, onChange, options }: React.ComponentProps<typeof DropDownDayPicker>) => {
			...
			return (
				<Select value={value?.toString()} onValueChange={handleChange}>
					<SelectTrigger className={cn(hideNavigation ? "" : "h-7 border-solid text-sm", props.selectTriggerClassName)}>
